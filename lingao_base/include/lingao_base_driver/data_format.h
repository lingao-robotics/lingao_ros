/*
 * Copyright (C) 2024, LingAo Robotics, INC.
 * @Version: V1.0
 * @Author: owen (keaa@keaa.net)
 * @Date: 2022-11-09 20:27:15
 * @LastEditTime: 2024-02-05 12:13:40
 * @LastEditors: owen
 * @Description: 
 * @FilePath: /lingao_ans/home/owen/workspace/lingao_ros2_ws/src/lingao_ros2/lingao_base/include/lingao_base_driver/data_format.h
 */
...
enum Message_Id_Enum
{
    ...
    MSG_ID_GET_STATUS   = 0x05, // 获取车体状态信息
    MSG_ID_GET_RC       = 0x06, // 获取遥控数据
    MSG_ID_SET_SOUND_LIGHT   = 0x07,     // 设置声光数据
    MSG_ID_GET_VER      = 0xFF
};
...
// 底盘状态数据结构体
struct Data_Format_Status
{
    uint8_t system_status;
    uint8_t anti_collision_status;
    uint8_t anti_drop_status;
    uint16_t temperature;
    uint16_t humidity;
    uint16_t barometer;
    uint8_t charging_pile;
    uint8_t reserve;

    void EndianSwapSet(const void *src)
    {
        memcpy(this, src, sizeof(system_status)*3);
        FormatTools::EndianSwap(&temperature, (char *)src+3, sizeof(temperature), sizeof(temperature)*3);
        memcpy(&charging_pile, (char *)src+9, sizeof(charging_pile)*2);
    }
};

// 声光数据结构体
struct Data_Format_Sound_Light
{
    uint8_t light_status;
    uint8_t light_red_color;
    uint8_t light_green_color;
    uint8_t light_blue_color;
    uint8_t beep_status;
    uint8_t searchlight_status;

    void EndianSwapSet(const void *src)
    {
        memcpy(this, src, sizeof(Data_Format_Sound_Light));
    }
};
